...
patent_config = {
    ...
    "tech_categories_queries": {
        "AI": (
            ...
            'AND (algorithm OR model OR neural OR network))'
        ),
        "Blockchain": (
            '(blockchain OR "distributed ledger" OR cryptocurrency OR "smart contract")'
        ),
        "QuantumComputing": '("quantum comput*" OR qubit OR "quantum algorithm")',
        "RenewableEnergy": (
            '("solar power" OR "wind energy" OR "battery storage" OR "renewable fuel")'
        ),
        "AutonomousVehicles": (
            '("self-driving car" OR "autonomous vehicle" OR lidar OR "adas")'
        ),
        "Biotechnology": (
            '("gene editing" OR crispr OR immunotherapy OR "synthetic biology")'
        ),
        "ARVR": (
            '("augmented reality" OR "virtual reality" OR "mixed reality" OR metaverse)'
        ),
        ...
        "CybersecurityTech": (
            '("cybersecurity" OR "threat detection" OR "zero trust" OR '
...
research_config = {
    ...
    "pubmed_search_terms": [
        (
            ...
            '(drug discovery OR diagnosis OR "medical imaging")'
        ),
        'gene therapy AND (clinical trial OR efficacy OR "vector design")',
        "CRISPR Cas9 applications",
        "immunotherapy advancements AND cancer",
        (
            '("renewable energy" OR solar OR wind) AND '
            '("storage" OR "grid integration")'
        ),
        (
            '("quantum computing" OR "quantum computer") AND ' # shortened
            '(algorithms OR "drug development" OR "materials science")'
        ),
        (
            '("autonomous vehicles" OR "self-driving car") AND ' # shortened
            '(safety OR "sensor fusion" OR "navigation system")'
        ),
        (
            '("augmented reality" OR "virtual reality") AND ' # shortened
            '(healthcare OR industrial OR training)'
        ),
        "robotics AND (surgical OR logistics OR manufacturing)",
        (
            '("cybersecurity" OR "cyber security") AND ' # shortened
            '("threat intelligence" OR "data privacy" OR "network security")'
        ),
    ],
...
model_config = {
    ...
    "prediction_horizons_months": [
        ...
        18,
        24,
    ],  # For which future periods to forecast
...
monitoring_config = {
    ...
    "pipeline_health_checks": {
        "patents_data_pipeline": {
            "max_staleness_days": 2,
            "expected_status": "SUCCESS",
        },
        "funding_data_pipeline": {
...
uncertainty_config = {
    ...
    "low_confidence_disclaimer": (
        "Significant uncertainties exist; interpret with extreme caution and "
        "seek expert consultation."
    ),
    "medium_confidence_disclaimer": (
        "Some uncertainties identified; consider these in your assessment and "
        "monitor developments."
    ),
    "high_confidence_disclaimer": (
        "While confidence is relatively high, predictions are inherently "
        "uncertain and subject to change."
    ),
}

# Ensure consistency for shared threshold labels
# This line directly assigns the dictionary from prediction_config,
# so if prediction_config['confidence_thresholds_labels'] is updated,
# uncertainty_config['confidence_threshold_labels'] will also reflect that.
uncertainty_config["confidence_threshold_labels"] = prediction_config[
    "confidence_thresholds_labels"
]
