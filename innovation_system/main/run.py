...
# from data_collection.collectors import ( # Not used directly in mock run
#     PatentDataCollector,
#     FundingDataCollector,
...
from config.settings import (
    general_config,
    # patent_config, # Not directly used in mock run.py
    # funding_config, # Not directly used in mock run.py
    # research_config, # Not directly used in mock run.py
    # feature_config, # Not directly used in mock run.py
    model_config,
    prediction_config,
...
logging.basicConfig(
    ...
    filemode="w",
)  # Overwrite log each run for demo purposes
...
if __name__ == "__main__":
    ...
    uncertainty_handler = UncertaintyManager(
        global_prediction_config=uncertainty_config,  # Corrected to use uncertainty_config
        logger_instance=logger.getChild("UncertaintyManager"),
    )

    # --- Data Collection (Mock/Conceptual) ---
    logger.info(
        "Data collection phase (conceptual - using mock data structures for demo)."
    )
    ...
    logger.info(
        "Feature engineering phase (conceptual - creating dummy historical features)."
    )
            ...
            hist_data_list.append(
                {
                    ...
                    "sector_id": sector,
                    "patent_filing_rate": np.random.rand()
                    + (0.1 * (sector_idx + 1)),  # Sector A higher trend
                    ...
                    "y_growth_6m_actual": np.random.normal(
                        loc=0.03 + (0.02 * sector_idx), scale=0.02
                    ),
                }
            )
    ...
    demo_feature_cols = [
        ...
        "funding_velocity",
        "research_output",
        "avg_citation_external_norm",
        "tech_diversity_shannon",
    ]
    ...
    logger.info(
        f"Generated {len(X_historical_demo)} dummy historical records for training across {len(sectors_demo)} sectors."
    )

    # --- Model Training ---
    logger.info("Starting model training...")
    innovation_model_trainer.train_all_sector_models(
        X_historical_demo.drop(columns=["sector_id"]),
        y_historical_demo,
        sector_id_column=X_historical_demo["sector_id"],
    )  # Pass sector_id as series
    logger.info("Model training complete.")
    logger.info(
        f"Trained models for sectors: {list(innovation_model_trainer.trained_sector_models.keys())}"
    )
    ...
    for (
        ...
        innovation_model_trainer.trained_sector_models.keys()
    ):  # Only validate for sectors with models
        ...
        if len(sector_data) > 10:  # Ensure enough data for a small "test"
            test_samples = sector_data.tail(
                min(5, len(sector_data) // 5)
            )  # e.g. last 5 samples or 20%
    ...
    if test_data_frames_X:
        ...
        logger.info(
            "Skipped conceptual validation due to insufficient data per sector post-training."
        )
    ...
    predictor_instance = PredictionGenerator(
        trained_models_info_for_pred,
    ...
    for sector_name in innovation_model_trainer.trained_sector_models.keys():
        last_known_features_raw = (
            X_historical_demo[X_historical_demo["sector_id"] == sector_name]
            .tail(1)
            .drop(columns=["sector_id"])
        )
    ...
    if current_features_map_demo:
        forecasts_output = predictor_instance.generate_forecasts_for_sectors(
            current_features_map_demo,
            horizons_list=model_config[
                "prediction_horizons_months"
            ],  # Global config from its module
        )
        ...
        logger.warning(
            "No current features available to generate forecasts in demo for trained sectors."
        )

    # --- Emergence & Investment Opportunities (Conceptual) ---
    logger.info(
        "Identifying emerging technologies and investment opportunities (conceptual)..."
    )
    ...
    if current_state_features_list:
        current_state_full_df = pd.concat(current_state_features_list).reset_index(
            drop=True
        )
        ...
        emergence_config_features = list(
            prediction_config.get("emergence_indicators_weights", {}).keys()
        )
        for f_name in emergence_config_features:
            if f_name not in current_state_full_df.columns:
                ...
                proxy_col = demo_feature_cols[
                    np.random.randint(0, len(demo_feature_cols))
                ]
                current_state_full_df[f_name] = (
                    current_state_full_df[proxy_col] * np.random.rand()
                )  # Scaled proxy
                logger.warning(
                    f"Emergence indicator '{f_name}' not in demo features, using scaled proxy '{proxy_col}'."
                )

        emerging_techs = predictor_instance.identify_emerging_technologies(
            current_state_full_df, sector_col="sector_id"
        )
        logger.info(
            f"Emerging Technologies Identified (sample): {str(emerging_techs)[:500]}..."
        )
        ...
        logger.info(
            f"Investment Opportunities (sample): {str(investment_ops)[:500]}..."
        )
    else:
        logger.warning(
            "Skipped emergence/investment analysis due to no current features."
        )
    ...
    system_monitor = SystemMonitor(
        trained_models_info_dict=trained_models_info_for_pred,
    ...
    if not baseline_data_X.empty:
        system_monitor.establish_drift_baselines(
            baseline_data_X, sector_id_col="sector_id"
        )

        # 3. Monitor for data drift using current features
        if current_features_map_demo:
            drift_alerts = system_monitor.monitor_for_data_drift(
                current_features_map_demo, predictor_instance
            )
            if drift_alerts:
                logger.warning(f"Data Drift Alerts: {drift_alerts}")
    else:
        logger.warning(
            "Could not establish drift baselines or monitor drift due to empty historical data."
        )
    ...
    if test_data_frames_X:  # If we created a mock test set
        for sector_eval_name in X_test_val_demo[
            "sector_id"
        ].unique():  # Renamed 'sector' to 'sector_eval_name'
            X_sector_eval_df = X_test_val_demo[
                X_test_val_demo["sector_id"] == sector_eval_name
            ].drop(
                columns=["sector_id"]
            )  # Renamed 'X_sector_eval'
            y_sector_eval_series = y_test_val_demo[
                y_test_val_demo.index.isin(X_sector_eval_df.index)
            ]  # Renamed 'y_sector_eval'
            if not X_sector_eval_df.empty and not y_sector_eval_series.empty:
                recent_eval_data_map_demo[sector_eval_name] = (
                    X_sector_eval_df,
                    y_sector_eval_series,
                )

    if recent_eval_data_map_demo:
        perf_issues = system_monitor.evaluate_recent_model_performance(
            recent_eval_data_map_demo, predictor_instance
        )
        if perf_issues:
            logger.warning(f"Model Performance Issues: {perf_issues}")
            for sector_issue in perf_issues.keys():  # Use keys() for clarity
                system_monitor.suggest_retraining_trigger(
                    sector_issue,
                    monitoring_config.get(
                        "retraining_suggestion_reason_performance",
                        "Performance degradation",
                    ),
                )
    else:
        logger.info(
            "Skipped recent model performance evaluation due to no mock recent data."
        )
    ...
    if forecasts_output and sectors_demo and sectors_demo[0] in forecasts_output:
        target_sector_for_uncertainty = sectors_demo[
            0
        ]  # Use the first demo sector if available

        # Check if this sector actually has forecasts
        if forecasts_output.get(target_sector_for_uncertainty):
            first_horizon_months = model_config["prediction_horizons_months"][
                0
            ]  # e.g. 6
            forecast_sample = forecasts_output[target_sector_for_uncertainty].get(
                f"{first_horizon_months}m"
            )
            ...
            if forecast_sample:  # Ensure the specific horizon forecast exists
                ...
                research_cov_mock = {
                    target_sector_for_uncertainty: 0.65
                }  # Coverage for the specific sector
                conflict_mock = {
                    "is_conflicting": np.random.choice([True, False]),
                    "severity_0_to_1": np.random.rand() * 0.5,
                }
                custom_factors_mock = [
                    {
                        "name": "market_sentiment_change",
                        "impact_factor": -0.05,
                        "rationale": "Recent negative news in related markets",
                    }
                ]

                final_score, label, disclaimer = (
                    uncertainty_handler.get_overall_confidence_assessment(
                ...
                logger.info(
                    f"Uncertainty Assessment for {target_sector_for_uncertainty} Forecast ({first_horizon_months}m): "
                    f"Final Score={final_score:.2f}, Label='{label}'"
                )
                logger.info(f"Disclaimer: {disclaimer}")
            else:
                logger.warning(
                    f"No forecast sample found for {target_sector_for_uncertainty} to assess uncertainty."
                )
        else:
            logger.warning(
                "No sector with forecasts available for uncertainty assessment demo."
            )

    else:
        logger.warning(
            "No forecasts were generated; skipping uncertainty assessment demo."
        )
